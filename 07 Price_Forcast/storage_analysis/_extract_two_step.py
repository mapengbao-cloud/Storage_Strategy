...
# 定位项目根目录（向上查找含 .env 的目录）
def _find_root(start):
    p = Path(start).resolve()
    for cand in [p, *p.parents]:
        if (cand / ".env").exists():
            return cand
    return Path(__file__).parent.parent.parent  # 回退到 07 的上一级
...
def main():
    ...
    out_path = ROOT / "_tmp_two_step.json"
    ...
    print(f"  线性回归 charge={linreg['slope']}*pv + {linreg['intercept']}  R^2={linreg['r2']}")
    print()
    print("步骤2 削峰填谷：")
