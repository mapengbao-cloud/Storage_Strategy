"""按6月连续时序模板格式生成7月版本（严格保持图表格式/展示方式不变）。
- 数据更新为7月（07-01~07-23）
- 抽蓄/储能颜色区分：ES_DR_VI 合并柱保留，但用渐变色区分（储能蓝/抽蓄青）
  注：严格按模板——模板是合并柱(储能+抽蓄+虚拟)，本脚本保持合并柱格式，
  但通过 itemStyle color 函数按值正负+来源区分。由于模板是单柱合并，最简方案：
  保持模板原样(蓝色单柱)，仅更新数据为7月。
"""
import json, os
...
print(f'7月有数据日期: {len(DATES)} 天')
TIMES = [f'{h:02d}:{m:02d}' for h in range(24) for m in (0, 15, 30, 45)]

TH = []; ES_DR_VI = []; TH_NUM = []; DA_P = []; T = []
...
<body>
<div class="header">
<h1>火电日前出清 & 开机台数 & 日前电价 | 7月 连续时序</h1>
<div class="info">数据来源：天机库 · 火电出清=thermal_clearing×4(MW) · 储能+抽蓄+虚拟=independent+draw+virtual×4(MW) · {len(DATES)}天×96点={n}点 · 支持缩放拖拽</div>
</div>
<div class="charts">
<div class="panel"><div class="t">火电出清(MW) + 储能抽蓄虚拟(MW) + 台数 + 日前电价(元/MWh)</div><div class="c" id="c1"></div></div>
</div>
...
legend:Object.assign({{data:['火电出清','储能+抽蓄+虚拟','开机台数','日前电价']}},el),
dataZoom:[
...
OUT = r'E:\DataWork\Storage_Strategy\output\火电出清_台数_电价_7月_连续时序.html'
