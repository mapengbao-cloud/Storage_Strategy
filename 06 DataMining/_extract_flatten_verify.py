...
def main():
    ...
    out_path = Path(__file__).parent.parent / "_tmp_flatten_verify.json"
    ...
    h = out["h3_overall"]
    print(f"  储能总 ↔ bs偏差: {h['corr_st_bsdev']}")
    print(f"  独立储能 ↔ bs偏差: {h['corr_ind_bsdev']}")
    print(f"  抽蓄 ↔ bs偏差: {h['corr_dr_bsdev']}")
    print(f"  火电 ↔ bs: {h['corr_thermal_bs']}")
    print()
    print("H4 充电↔午间光伏:")
